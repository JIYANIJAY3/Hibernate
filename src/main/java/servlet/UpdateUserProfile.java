...
//		String password = request.getParameter("password");
		...
		userBean.setUserId(Integer.parseInt(userId.trim()));
...
//		userBean.setPassword(password);
		...
		for (int i = 0; i < country.length; i++) {
			...
			if (AddressId[i].isEmpty()) {
				...
				addressBean1.setUserId(Integer.parseInt(userId.trim()));
		...
		List<Integer> UserAddtessIdList = userInterface.getUserAddressId(conn, Integer.parseInt(userId.trim()));
		...
		for (Part filePart : userProfile) {
			if (filePart != null && filePart.getSize() != 0) {
				...
				userprofile.setUserId(Integer.parseInt(userId.trim()));
		...
		int deletUserAddress = userInterface.deleteUserAddress(conn, addressList3, Integer.parseInt(userId.trim()));
		...
		if (status > 0) {
			out.print("Update");
			List<UserProfileBean> profilelist = profileInterface.getUserImg(conn, Integer.parseInt(userId.trim()));
